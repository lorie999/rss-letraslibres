...
for seccio in SECCIONS:
    ...
    articles = sopa.find_all("article") or sopa.find_all("div", class_="post")

    if not articles:
        # Intentem buscar per estructura alternativa
        articles = sopa.select("div.entry-content, div.td-module-meta-info")
    ...
    for article in articles:
        ...
        if img_tag:
            ...
            if imatge and "110x110" in imatge:
                imatge = imatge.replace("110x110", "300x225")

        # Busquem el subtítol just després del títol
        subtitol_tag = titol_tag.find_next_sibling("p") or titol_tag.find_next("p")
        if subtitol_tag:
